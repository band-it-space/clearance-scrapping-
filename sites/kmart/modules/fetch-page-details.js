...
axiosRetry(axios, {
    ...
    retryDelay: (retryCount, error) => {
        ...
        console.warn(`Retry attempt ${retryCount} for ${url}: ${error.message || error}`);
        return 2000;
    },
...
export async function getPageDetails(url) {
    ...
    try {
        ...
        console.warn(`Initial retries failed for ${url}. Starting manual proxy fallback...`);
    ...
    try {
        ...
        console.error(`Extra data scrapping error for ${url}: ${error.message || error}`);
        return {
            success: false,
            data: {
                Description: "N/A",
